    ...
    T: num_traits::Float + Display,
{
    if arg2 == T::zero() {
        ...
        return err!(AdrenaError::MathOverflow);
    }
    ...
    T: Display + num_traits::ToPrimitive + Clone,
{
    ...
    if let Some(res) = option {
        ...
        msg!("Error: Overflow in {} as i32", arg);
        err!(AdrenaError::MathOverflow)
    }
}
...
pub fn scale_to_exponent(arg: u64, exponent: i32, target_exponent: i32) -> Result<u64> {
    ...
    if delta > 0 {
        ...
        if result * checked_pow(10, delta as usize)? < (arg - (arg / Cortex::BPS_POWER as u64)) {
            return Err(AdrenaError::PythPriceExponentTooLargeIncurringPrecisionLoss.into());
        }

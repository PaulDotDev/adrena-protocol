...
pub fn liquidate_short(ctx: Context<LiquidateShort>, params: &LiquidateShortParams) -> Result<()> {
    ...
    // Check there are enough tokens available to pay for reward + user tokens
    require!(
    ...
    {
        collateral_custody.collected_fees.liquidation_usd += exit_numbers.exit_fee_usd;
        ...
        collateral_custody.assets.collateral -= position.collateral_amount;

        custody.volume_stats.liquidation_usd += position.size_usd;

        custody.trade_stats.oi_short_usd -= position.size_usd;

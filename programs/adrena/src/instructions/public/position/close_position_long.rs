...
pub fn close_position_long(
    ...
    // Check exit price slippage
    {
        ...
        if let Some(price) = params.price {
            require_gte!(exit_price, price, AdrenaError::MaxPriceSlippage);
        }
    }
    ...
    custody.unlock_funds(position_to_close.locked_amount)?;
    ...
    // Update custody stats
    {
        ...
        custody.collected_fees.close_position_usd += exit_numbers.exit_fee_usd;
        ...
        custody.assets.collateral -= position_to_close.collateral_amount;
        ...
        custody.trade_stats.oi_long_usd -= position_to_close.size_usd;

        // If it's a partial close, trick it by doing +1 to nullify the -1 in update_accounting_after_remove_position_long
        if is_partial_close {
            custody.long_positions.open_positions += 1;
        }

        custody.update_accounting_after_remove_position_long(&position_to_close, current_time)?;
    }

...
pub fn add_liquid_stake(ctx: Context<AddLiquidStake>, params: &AddLiquidStakeParams) -> Result<()> {
    ...
    // Add stake to UserStaking account
    {
        ...
        if user_staking.liquid_stake.amount > 0 {
            ...
            user_staking = ctx.accounts.user_staking.load_mut()?;

            // If the user has rewards for the current round, needs to set overlap attribute
            // So we know how much of the stake is eligible for rewards
            {
                staking = ctx.accounts.staking.load_mut()?;

                if user_staking
                    .liquid_stake
                    .qualifies_for_rewards_from(&staking.current_staking_round)
                {
                    user_staking.liquid_stake.overlap_time = cortex.get_time()?;
            ...
            user_staking.liquid_stake.stake_time = cortex.get_time()?;
        }

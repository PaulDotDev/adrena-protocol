...
#[derive(Accounts)]
pub struct InitStakingTwo<'info> {
    /// #1
    ...
    #[account(
        ...
        constraint = staking.load()?.get_initialized() == StakingInitializationStep::Step1 @AdrenaError::InvalidStakingState
    )]
...
pub fn init_staking_two<'info>(
    ...
    staking.reward_token_vault_bump = ctx.bumps.staking_reward_token_vault;

    staking.initialized = StakingInitializationStep::Step2.into();

use {
    ...
    adrena::{
        instructions::{DistributeFeesParams, RemoveLiquidityParams},
        state::pool::Pool,
    },
    anchor_lang::{
...
pub async fn remove_liquidity(
    ...
    let accounts_meta = {
        let accounts = adrena::accounts::RemoveLiquidity {
            owner: owner.pubkey(),
    ...
    let (ix_size, tx_used_cu) = utils::create_and_execute_adrena_ix(
        program_test_ctx,
        accounts_meta,
        adrena::instruction::RemoveLiquidity {
            params: params.clone(),
        },
        Some(&payer.pubkey()),
        &[owner, payer],
    ...
    utils::log_instruction("remove_liquidity", "public", ix_size, tx_used_cu);

use {
    ...
    adrena::{instructions::ExecuteLimitOrderLongParams, state::position::Side},
...
pub async fn execute_limit_order_long(
    ...
    let collateral_escrow_pda =
        pda::get_collateral_escrow_pda(pool_pda, &owner.pubkey(), custody_token_mint).0;
    ...
    let (ix_size, tx_used_cu) = utils::create_and_execute_adrena_ix(
        program_test_ctx,
        adrena::accounts::ExecuteLimitOrderLong {
            owner: owner.pubkey(),
        ...
        adrena::instruction::ExecuteLimitOrderLong {
            params: params.clone(),
        },
        Some(&payer.pubkey()),
        &[caller, payer],
    ...
    utils::log_instruction("execute_limit_order_long", "public", ix_size, tx_used_cu);

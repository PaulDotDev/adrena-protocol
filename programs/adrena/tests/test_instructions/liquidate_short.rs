use {
    ...
    adrena::{
        instructions::{DistributeFeesParams, LiquidateShortParams},
        state::{
...
pub async fn liquidate_short(
    ...
    let collateral_custody_mint = collateral_custody_account.mint;
    ...
    let receiving_account_address =
        utils::find_associated_token_account(&owner, &collateral_custody_mint).0;
    ...
    let (ix_size, tx_used_cu) = utils::create_and_execute_adrena_ix(
        program_test_ctx,
        adrena::accounts::LiquidateShort {
        ...
        adrena::instruction::LiquidateShort {
        ...
        Some(vec![
            get_distribute_fees_ix(
                ...
                payer,
                &position.pool,
                &DistributeFeesParams { oracle_prices },
            )
            .await?,
        ]),
    )
    ...
    utils::log_instruction("liquidate_short", "public", ix_size, tx_used_cu);

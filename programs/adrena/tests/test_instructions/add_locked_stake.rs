...
pub async fn add_locked_stake(
    ...
    let staking_reward_token_account_address = utils::find_associated_token_account(
        &owner.pubkey(),
    ...
    let (ix_size, tx_used_cu) = utils::create_and_execute_adrena_ix(
        program_test_ctx,
        adrena::accounts::AddLockedStake {
            owner: owner.pubkey(),
        ...
        adrena::instruction::AddLockedStake {
            params: AddLockedStakeParams {
    ...
    utils::log_instruction("add_locked_stake", "public", ix_size, tx_used_cu);
    ...
    // Check voting power
    {
        // print params.locked_days;
        println!("params.locked_days: {}", params.locked_days);
        ...
        let additional_voting_power = math::checked_as_u64(
            (params.amount * staking_option.vote_multiplier as u64) as u128 / Cortex::BPS_POWER,
        )
        .unwrap();

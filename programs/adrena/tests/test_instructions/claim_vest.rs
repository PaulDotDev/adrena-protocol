...
pub async fn claim_vest(
    ...
    let vest_account_before =
        utils::get_zero_copy_account::<Vest>(program_test_ctx, vest_pda).await;
    ...
    utils::log_instruction("claim_vest", "public", ix_size, tx_used_cu);

    // ==== THEN ==============================================================
    let vest_account_after = utils::get_zero_copy_account::<Vest>(program_test_ctx, vest_pda).await;
    ...
    // If everything have been claimed, verify that the vest pda has been plucked out of the vest_registry vests vector
    if vest_account_after.amount == vest_account_after.claimed_amount {
        assert_eq!(
            vest_registry_account_after
                .vests
                .iter()
                .find(|v| { **v == vest_pda }),
            None
    ...
    assert_eq!(
        vest_registry_account_before.vesting_token_amount - total_claim,

use {
    ...
    adrena::{
        instructions::GetAssetsUnderManagementParams,
...
pub async fn get_assets_under_management(
    program_test_ctx: &RwLock<ProgramTestContext>,
    payer: &Keypair,
    pool_pda: &Pubkey,
    oracle_prices: Option<ChaosLabsBatchPrices>,
) -> std::result::Result<u128, BanksClientError> {
    // ==== WHEN ==============================================================
    ...
    let accounts_meta = {
        let accounts = adrena::accounts::GetAssetsUnderManagement {
            cortex: cortex_pda,
            pool: *pool_pda,
            oracle: oracle_pda,
        };
    ...
    let result: u128 = utils::create_and_simulate_cortex_view_ix(
        program_test_ctx,
        accounts_meta,
        adrena::instruction::GetAssetsUnderManagement {

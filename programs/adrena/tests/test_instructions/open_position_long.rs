use {
    ...
    adrena::{
        instructions::OpenPositionLongParams,
...
pub async fn open_position_long(
    ...
    let (ix_size, tx_used_cu) = utils::create_and_execute_adrena_ix(
        program_test_ctx,
        adrena::accounts::OpenPositionLong {
            owner: owner.pubkey(),
            caller: owner.pubkey(),
            payer: payer.pubkey(),
        ...
        adrena::instruction::OpenPositionLong {
            params: params.clone(),
        },
        Some(&payer.pubkey()),
        &[owner, payer],
    ...
    utils::log_instruction("open_position_long", "public", ix_size, tx_used_cu);
    ...
    // Check the position
    {
        ...
        assert!(position_account.collateral_amount <= params.collateral); // Due to entry fees - <= to accomodate tests without fees

use {
    ...
    adrena::{
        instructions::PoolInfoSnapshot,
        state::{chaos_labs_oracle::ChaosLabsBatchPrices, pool::Pool},
    },
...
pub async fn get_pool_info_snapshot(
    program_test_ctx: &RwLock<ProgramTestContext>,
    payer: &Keypair,
    pool_pda: &Pubkey,
    lp_token_mint_pda: &Pubkey,
    oracle_prices: Option<ChaosLabsBatchPrices>,
) -> std::result::Result<PoolInfoSnapshot, BanksClientError> {
    // ==== WHEN ==============================================================
    let oracle_pda = pda::get_oracle_pda().0;
    let cortex_pda = pda::get_cortex_pda().0;

    let accounts_meta = {
        let accounts = adrena::accounts::GetPoolInfoSnapshot {
            cortex: cortex_pda,
            pool: *pool_pda,
    ...
    let result: PoolInfoSnapshot = utils::create_and_simulate_cortex_view_ix(
        program_test_ctx,
        accounts_meta,
        adrena::instruction::GetPoolInfoSnapshot {
            params: adrena::instructions::GetPoolInfoSnapshotParams {

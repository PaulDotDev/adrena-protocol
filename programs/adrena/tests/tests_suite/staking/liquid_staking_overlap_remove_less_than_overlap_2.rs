...
// Check removing less than the overlap just after the overlap
// Vs liquid_staking_overlap_remove_less_than_overlap that removes the overlap after a staking round passed
pub async fn liquid_staking_overlap_remove_less_than_overlap_2() {
    ...
    // Move to half the round
    utils::warp_forward(
        &test_setup.program_test_ctx,
        StakingRound::ROUND_MIN_DURATION_SECONDS / 2,
    )
    .await;
    ...
    // Remove LESS than the overlap staking
    {
        ...
        test_instructions::remove_liquid_stake(
            ...
            alice,
            &test_setup.payer_keypair,
            RemoveLiquidStakeParams { amount: 500000 },
    ...
    // Claim rewards
    // alice should get rewards from both staking
    {
        let balance_before = utils::get_token_account_balance(
            &test_setup.program_test_ctx,
            alice_staking_reward_token_account_address,

...
pub async fn auto_claim() {
    ...
    let mut automation_executions: Vec<u64> = Vec::new();
    ...
    for i in 0..30 {
        ...
        // Check if automation can trigger
        {
            let has_automation_been_executed = utils::execute_claim_stakes_automation(
                &test_setup.program_test_ctx,
                &test_setup.payer_keypair,
                &alice.pubkey(),
                &test_setup.pool_pda,
            ...
            if has_automation_been_executed {
                automation_executions.push(i);
            }
        }
    }
    ...
    assert!(!automation_executions.is_empty());

    println!("Automation executions: {:?}", automation_executions);
}

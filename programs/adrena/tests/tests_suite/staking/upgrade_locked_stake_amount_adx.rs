...
pub async fn upgrade_locked_stake_amount_adx() {
    ...
    // Upgrade 0 the stake amount (should fail)
    {
    ...
    // Claim when there is one round worth of rewards to claim
    {
        ...
        // Alice has 1x180d and martin 2x180d - so alice is entitled to 33% rewards
        assert_eq!(balance_after - balance_before, 25000);
        assert_eq!(lm_balance_after - lm_balance_before, 23888888888);
    }

    // Upgrade 1 the stake amount (totalling 2)
    {
    ...
    // Claim when there is one round worth of rewards to claim
    {
        ...
        // Alice has 2x180d and martin 2x180d - so alice is entitled to 50% rewards
...
async fn finalize_and_remove_stake(
    ...
    // Move 180d in the future where staking have ended
    utils::warp_forward(&test_setup.program_test_ctx, utils::days_in_seconds(180)).await;
...
async fn prep_work(
    ...
    // Claim when there is one round worth of rewards to claim
    {
        ...
        assert_eq!(lm_balance_after - lm_balance_before, 47777777777);
    }

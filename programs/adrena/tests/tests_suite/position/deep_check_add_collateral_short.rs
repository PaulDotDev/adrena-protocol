...
pub async fn deep_check_add_collateral_short() {
    ...
    // Simple add collateral short (with mandatory open position (Short 1 ETH 10x leverage, using USDC as collateral))
    {
        ...
        let collateral_usdc = utils::scale(1500, USDC_DECIMALS);
        let added_collateral_usdc = utils::scale(1500, USDC_DECIMALS);
        let leverage = 10;
        ...
        // Alice: Add 1500 USDC collateral to short position
        test_instructions::add_collateral_short(
            ...
            AddCollateralShortParams {
                collateral: added_collateral_usdc,
                oracle_prices: Some(
                    utils::get_oracle_prices_as_chaos_labs_bundle(&test_setup.program_test_ctx)
                        .await,
                ),
        ...
        let exit_fee = 24000000;
        let liquidation_fee = 24000000;

        let collateral_usdc_usd_value = 1498500000; // with confidence taken into account (1 usd =/= 1 usdc)
        let added_collateral_usdc_usd_value = 1498500000; // with confidence taken into account (1 usd =/= 1 usdc)
        ...
        // Check position
        {
            ...
            assert_eq!(position.size_usd, (collateral_usdc_usd_value) * leverage);
            assert_eq!(
                position.borrow_size_usd,
                (collateral_usdc_usd_value) * leverage
            );
            assert_eq!(
                position.collateral_usd,
                collateral_usdc_usd_value + added_collateral_usdc_usd_value
            );
            assert_eq!(position.unrealized_interest_usd, 0);
            assert_eq!(position.cumulative_interest_snapshot, U128Split::default());
            assert_eq!(position.locked_amount, (collateral_usdc) * leverage); // 100% of collateral provided for now, minus fees
            assert_eq!(
                position.collateral_amount,
                collateral_usdc + added_collateral_usdc
            );
            assert_eq!(position.exit_fee_usd, exit_fee);
            assert_eq!(position.liquidation_fee_usd, liquidation_fee);
        }
        ...
        // Check Alice token accounts
        {
            ...
            // Alice paid 1 ETH worth in USDC minus fees
            assert_eq!(
        ...
        // Check custodies
        {
            ...
            // Asset owned must match the actual amount in the token account
            {
                ...
                assert_eq!(
                    ...
                    Assets {
                        ...
                        collateral: collateral_usdc + added_collateral_usdc,
                        owned,
                ...
                assert_eq!(
                    ...
                    owned + (collateral_usdc) + added_collateral_usdc
                );
            }
            ...
            assert_eq!(
                ...
                PositionsAccounting {
                    ...
                    borrow_size_usd: 0,
                    locked_amount: 0,
                    weighted_price: U128Split::new(1498500000000000000),
                    total_quantity: U128Split::new(99900),
                    cumulative_interest_usd: 0,
                    collateral_usd: 0,
                    cumulative_interest_snapshot: U128Split::new(0),
                    exit_fee_usd: real_position_size * 16 / Cortex::BPS_POWER as u64,
                    stable_locked_amount: [

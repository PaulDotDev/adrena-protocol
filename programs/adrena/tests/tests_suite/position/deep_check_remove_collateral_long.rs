use {
    ...
    adrena::{
        instructions::{InitUserProfileParams, OpenPositionLongParams, RemoveCollateralLongParams},
        state::{
            cortex::Cortex,
            custody::{
...
pub async fn deep_check_remove_collateral_long() {
    ...
    // Simple add collateral long (1ETH) (and mandatory previous open position (Long 1 ETH 10x leverage))
    {
        // GIVEN --------------------------------------------------------------
        let eth_max_open_price_usdc = utils::scale(1_550, Cortex::PRICE_DECIMALS);
        let collateral_eth = utils::scale(1, ETH_DECIMALS);
        // let removed_collateral_eth = 100_000_000; // 0.1 ETH
        let removed_collateral_usd = utils::scale(150, Cortex::USD_DECIMALS);
        ...
        // WHEN ---------------------------------------------------------------
        // Alice: Open 1 ETH Long position
        ...
        // Alice: Removes 100$ of ETH collateral from long position
        test_instructions::remove_collateral_long(
            &test_setup.program_test_ctx,
            alice,
            &test_setup.payer_keypair,
            &position_pda,
            RemoveCollateralLongParams {
        ...
        // Check position
        {
            assert_eq!(position.get_side(), position::Side::Long);
            ...
            assert_eq!(position.collateral_custody, eth_custody_pda);
            ...
            assert_eq!(
                ...
                collateral_usd_value - removed_collateral_usd
            );
            assert_eq!(position.unrealized_interest_usd, 0);
            assert_eq!(position.cumulative_interest_snapshot, U128Split::default());
            assert_eq!(position.locked_amount, (collateral_eth) * leverage); // 100% of collateral provided for now, minus fees
            assert_eq!(
                position.collateral_amount,
                900249377, // TODO: collateral_eth - removed_collateral_eth (900000000) : Needs to account for high/low confidence
            );
        ...
        // Check Alice token accounts
        {
            ...
            // Alice paid 1 ETH minus fees
            assert_eq!(
                alice_eth_before - alice_eth_after,
                900249377, // TODO: collateral_eth - removed_collateral_eth (900000000) : Needs to account for high/low confidence
            );
        ...
        // Check custodies
        {
            ...
            // initial_add_liquidity_usdc_amount_usd * 90 / Cortex::BPS_POWER as u64;
            // --------------------------------------------------------------------------------
            ...
            assert_eq!(
                ...
                TradeStats {
                    profit_usd: 0,
                    loss_usd: 0,
                    oi_long_usd: real_position_size_usd,
                    oi_short_usd: 0,
                },
            );
            assert_eq!(
            ...
            // Asset owned must match the actual amount in the token account
            {
                ...
                assert_eq!(
                    ...
                    Assets {
                        collateral: 900249377, // TODO: collateral_eth - removed_collateral_eth, Needs to account for high/low confidence
                        owned,
                        locked: real_position_size,
                    },
                );

                // All that's been deposited
                assert_eq!(
                    eth_custody_token_account_after.amount,
                    initial_add_liquidity_eth_amount + 900249377, // TODO: collateral_eth - removed_collateral_eth, Needs to account for high/low confidence
                );
            }
            ...
            assert_eq!(
                ...
                PositionsAccounting {
                    ...
                    weighted_price: U128Split::new(1496250000000000000),
                    total_quantity: U128Split::new(99750),
                    cumulative_interest_usd: 0,
                    collateral_usd: collateral_usd_value - removed_collateral_usd,
            ...
            assert_eq!(eth_custody_account.borrow_rate_state.current_rate, 455);

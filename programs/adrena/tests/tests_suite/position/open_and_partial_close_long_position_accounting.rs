...
#[allow(deprecated)]
pub async fn open_and_partial_close_long_position_accounting() {
    ...
    // Martin: Partially close the SOL position
    close_position_long(
        &test_setup.program_test_ctx,
        martin,
        &test_setup.payer_keypair,
...

    {
        ...
        // Double check effect of closing position on jitoSOL custody accounting
        {
            // Collected fees
            {
                ...
                assert_eq!(after.close_position_usd - before.close_position_usd, 960000,);

                assert_eq!(after.borrow_usd - before.borrow_usd, 29925);
            ...
            // Trade Stats
            {
                ...
                assert_eq!(after.loss_usd - before.loss_usd, 60839925);

                assert_eq!(after.oi_long_usd, 399000000);
                assert_unchanged!(before.profit_usd, after.profit_usd);
                assert_unchanged!(before.oi_short_usd, after.oi_short_usd);
            }
            ...
            // Long positions
            {
                ...
                // locked amount (size) * position price (entry price)
                assert_eq!(before.size_usd - after.size_usd, 598500000);

                // locked amount (size) * position price (entry price)
                assert_eq!(before.borrow_size_usd - after.borrow_size_usd, 598500000);

                assert_eq!(before.locked_amount - after.locked_amount, 3000000000);

                assert_eq!(
                    before.total_quantity - after.total_quantity,
                    U128Split::from(59850)
                );

                assert_eq!(
                    before.weighted_price - after.weighted_price,
                    U128Split::from(59850000000000000_u128)
                );

                assert_eq!(
                    after.cumulative_interest_snapshot - before.cumulative_interest_snapshot,
                    U128Split::from(50000)
                );

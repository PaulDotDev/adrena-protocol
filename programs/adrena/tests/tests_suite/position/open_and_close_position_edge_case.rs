use {
    ...
    adrena::{
        instructions::{
            ClosePositionLongParams, ClosePositionShortParams, InitUserProfileParams,
            OpenPositionLongParams, OpenPositionShortParams, SetStopLossLongParams,
            SetStopLossShortParams, SetTakeProfitLongParams, SetTakeProfitShortParams,
        },
        state::{
            cortex::Cortex,
...
#[allow(deprecated)]
pub async fn open_and_close_position_edge_case() {
    ...
    // Long: Close a position when a take profit is in place
    {
        ...
        utils::warp_forward(&test_setup.program_test_ctx, 100).await;

        test_instructions::set_take_profit_long(
            &test_setup.program_test_ctx,
            martin,
            &test_setup.payer_keypair,
            &position_pda,
            SetTakeProfitLongParams {
                take_profit_limit_price: utils::scale(120, Cortex::PRICE_DECIMALS),
            },
        )
        .await
        .unwrap();
    ...
    // Long: Close a position when a stop_loss is in place
    {
        let position_pda = open_position_long(
            &test_setup.program_test_ctx,
            martin,
        ...
        test_instructions::set_stop_loss_long(
            &test_setup.program_test_ctx,
            martin,
            &test_setup.payer_keypair,
            &position_pda,
            SetStopLossLongParams {
                stop_loss_limit_price: utils::scale(80, Cortex::PRICE_DECIMALS),
                close_position_price: Some(utils::scale(70, Cortex::PRICE_DECIMALS)),
            },
        )
        .await
        .unwrap();
    ...
    // Short: Close a position when a take profit is in place
    {
        ...
        test_instructions::set_take_profit_short(
            ...
            SetTakeProfitShortParams {
                take_profit_limit_price: utils::scale(90, Cortex::PRICE_DECIMALS),
            },
        )
        .await
        .unwrap();
        ...
        test_instructions::close_position_short(
            ...
            ClosePositionShortParams {
                price: Some(utils::scale(120, Cortex::PRICE_DECIMALS)),
                oracle_prices: Some(
                    utils::get_oracle_prices_as_chaos_labs_bundle(&test_setup.program_test_ctx)
                        .await,
                ),
    ...
    // Short: Close a position when a stop loss is in place
    {
        let position_pda = test_instructions::open_position_short(
            &test_setup.program_test_ctx,
            martin,
        ...
        test_instructions::set_stop_loss_short(
            ...
            SetStopLossShortParams {
                stop_loss_limit_price: utils::scale(90, Cortex::PRICE_DECIMALS),
                close_position_price: Some(utils::scale(90, Cortex::PRICE_DECIMALS)),
            },
        )
        .await
        .unwrap();

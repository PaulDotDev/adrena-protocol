...
pub async fn deep_check_open_position_short() {
    ...
    // Simple open position (Short 1 ETH 10x leverage, using USDC as collateral)
    {
        // GIVEN --------------------------------------------------------------
        let eth_max_open_price_usdc = utils::scale(1_450, Cortex::PRICE_DECIMALS);
        let collateral_usdc = 1500000000;
        let leverage = 10;
        ...
        let exit_fee = 24000000; // 24 $USDC
        let liquidation_fee = 24000000; // 24 $USDC
        ...
        // Check position
        {
            ...
            assert_eq!(position.collateral_usd, collateral_usdc_usd_value);
            assert_eq!(position.unrealized_interest_usd, 0);
            assert_eq!(position.cumulative_interest_snapshot, U128Split::default());
            assert_eq!(position.locked_amount, (collateral_usdc) * leverage);
            assert_eq!(position.collateral_amount, collateral_usdc);
        ...
        // Check Alice token accounts
        {
            ...
            // Alice paid 1 ETH worth in USDC
            assert_eq!(alice_usdc_before - alice_usdc_after, collateral_usdc); // 1500$
        ...
        // Check custodies
        {
            ...
            let real_position_size_usdc = (collateral_usdc) * leverage;
            ...
            // Asset owned must match the actual amount in the token account
            {
                ...
                assert_eq!(
                    ...
                    Assets {
                        collateral: collateral_usdc,
                        owned,
                        locked: real_position_size_usdc, // It's a short, so we lock the full position size for 100% max profit
                    },
                );

                assert_eq!(
                    usdc_custody_token_account_after.amount,
                    owned + (collateral_usdc)
                );
            }
            ...
            assert_eq!(
                ...
                PositionsAccounting {
                    ...
                    collateral_usd: 0,
                    cumulative_interest_snapshot: U128Split::new(0),
                    exit_fee_usd: real_position_size_usdc * 16 / Cortex::BPS_POWER as u64,
                    stable_locked_amount: [
                        StableLockedAmountStat {
                            custody: usdc_custody_pda,
                            locked_amount: real_position_size_usdc,

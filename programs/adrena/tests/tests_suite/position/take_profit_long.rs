use {
    ...
    adrena::{
        instructions::{InitUserProfileParams, OpenPositionLongParams, SetTakeProfitLongParams},
        state::{
            cortex::Cortex,
...
pub async fn take_profit_long() {
    ...
    // Martin: Open 1 SOL long position x5
    let position_pda = test_instructions::open_position_long(
        ...
        OpenPositionLongParams {
            ...
            price: utils::scale(105, Cortex::PRICE_DECIMALS),
    ...
    // Take Profit fails when limit is not reached (should fail)
    {
        test_setup
        ...
        // should fail
        assert!(
            !utils::execute_take_profit_long_automation(
                &test_setup.program_test_ctx,
                &position_pda,
                &test_setup.payer_keypair,
                Some(
    ...
    // Cancel take profit (pause) then try to trigger it (should fail)
    {
        test_instructions::cancel_take_profit(
            &test_setup.program_test_ctx,
            martin,
        ...
        // Trigger Take Profit (should fail)
        {
            test_setup
                .update_oracle_price("sol", utils::scale(121, Cortex::PRICE_DECIMALS), 0)
                .await;

            utils::warp_forward(&test_setup.program_test_ctx, 1).await;

            // should fail
            assert!(!utils::execute_take_profit_long_automation(
                &test_setup.program_test_ctx,
                &position_pda,
                &test_setup.payer_keypair,
                Some(
    ...
    // Martin: update TP on the position
    test_instructions::set_take_profit_long(
        &test_setup.program_test_ctx,
        martin,
        &test_setup.payer_keypair,
        &position_pda,
        SetTakeProfitLongParams {
            take_profit_limit_price: utils::scale(119, Cortex::PRICE_DECIMALS),
        },
    )
    .await
    .unwrap();

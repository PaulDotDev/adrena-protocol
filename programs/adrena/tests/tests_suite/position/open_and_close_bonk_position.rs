...
#[allow(deprecated)]
pub async fn open_and_close_bonk_position() {
...

    {
        ...
        // Check user balance
        {
            // Paid 1000k bonk as collateral
            assert_eq!(
                martin_bonk_ata_balance_before - martin_bonk_ata_balance_after,
                utils::scale(10000000, BONK_DECIMALS),
            );
        }
        ...
        // Check the position PDA info
        {
            ...
            assert_eq!(position.price, 170329);
            assert_eq!(position.size_usd, 849520000);
            assert_eq!(position.borrow_size_usd, 849520000);
            // 1000k bonk at price
            assert_eq!(position.collateral_usd, 169904000);
            // 1000k bonk
            assert_eq!(
            ...
            assert_eq!(position.locked_amount, 5000000000000);

            assert_eq!(position.exit_fee_usd, 1362632);
            assert_eq!(position.liquidation_fee_usd, 1362632);
        }
        ...
        // Double check effect of opening position on ETH custody accounting
        {
            ...
            // Trade Stats
            {
                ...
                assert_eq!(after.oi_long_usd - before.oi_long_usd, 849520000);
            ...
            // Long positions
            {
                ...
                assert_eq!(after.size_usd - before.size_usd, 849520000);

                assert_eq!(after.borrow_size_usd - before.borrow_size_usd, 849520000);

                assert_eq!(after.locked_amount - before.locked_amount, 5000000000000);

                assert_eq!(
                    after.total_quantity - before.total_quantity,
                    U128Split::from(498752414444_u64)
                );

                // WeightedPrice = position_price * quantity
                assert_eq!(
                    after.weighted_price - before.weighted_price,
                    U128Split::from(84951999999832076_u128)
                );
...

    {
        ...
        // Double check effect of closing position on bonk custody accounting
        {
            // Collected fees
            {
                ...
                assert_eq!(after.borrow_usd - before.borrow_usd, 424760);
            ...
            // Trade Stats
            {
                ...
                assert_eq!(after.loss_usd - before.loss_usd, 86739891);
            ...
            // Long positions
            {
                ...
                // locked amount (size) * position price (entry price)
                assert_eq!(before.size_usd - after.size_usd, 849520000);

                // locked amount (size) * position price (entry price)
                assert_eq!(before.borrow_size_usd - after.borrow_size_usd, 849520000);

                assert_eq!(before.locked_amount - after.locked_amount, 5000000000000);

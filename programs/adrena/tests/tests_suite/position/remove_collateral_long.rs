...
pub async fn remove_collateral_long() {
        ...
        vec![
            utils::UserParam {
                ...
                token_balances: hashmap! {
                    "usdc" => utils::scale(100_000_000, USDC_DECIMALS),
                    "eth" => utils::scale(100_000, ETH_DECIMALS),
                },
            },
            utils::UserParam {
                name: "martin",
    ...
    // Martin: Open 1 ETH long position x1.1001
    let position_pda = test_instructions::open_position_long(
        ...
        OpenPositionLongParams {
            ...
            collateral: utils::scale(100, ETH_DECIMALS),
            leverage: 11_001,
            oracle_prices: Some(
    ...
    // position within min_leverage <> max_leverage and remove collateral within min_leverage <> max_leverage
    {
        test_instructions::remove_collateral_long(
            ...
            RemoveCollateralLongParams {
                collateral_usd: utils::scale(1_550, Cortex::USD_DECIMALS),
                oracle_prices: Some(
        ...
        assert_eq!(
            martin_usdc_balance,
            utils::scale(100_000_000, Cortex::USD_DECIMALS)
        );
        assert_eq!(martin_eth_balance, 9901030756442)
    }
    ...
    {
        assert!(test_instructions::remove_collateral_long(
            &test_setup.program_test_ctx,
            martin,
            &test_setup.payer_keypair,
            &position_pda,
            RemoveCollateralLongParams {
                collateral_usd: utils::scale(1_550_000, Cortex::USD_DECIMALS),
                oracle_prices: Some(
                    utils::get_oracle_prices_as_chaos_labs_bundle(&test_setup.program_test_ctx)
                        .await,
                ),
    ...
    // Remove very little
    {
        ...
        assert_eq!(martin_eth_balance, 9901031421446)
    }
    ...
    // Remove all collateral - 1 so max laverage exceeded
    {
        assert!(test_instructions::remove_collateral_long(
            &test_setup.program_test_ctx,
            martin,
    ...
    // Remove 0 collateral_usd
    {
        assert!(test_instructions::remove_collateral_long(
            ...
            &position_pda,
            RemoveCollateralLongParams {
                collateral_usd: utils::scale(0, Cortex::USD_DECIMALS),
                oracle_prices: Some(
                    utils::get_oracle_prices_as_chaos_labs_bundle(&test_setup.program_test_ctx)
                        .await,
                ),
    ...
    {
        // Make ETH price to drop in order to make the opened position leverage go above max leverage
        test_setup
            .update_oracle_price(
                "eth",
                utils::scale(20, Cortex::PRICE_DECIMALS),
                200000000, // 10bps
            )
            .await;
        ...
        // position above max leverage and remove collateral above max leverage
        assert!(test_instructions::remove_collateral_long(
            &test_setup.program_test_ctx,
            martin,
            &test_setup.payer_keypair,
            &position_pda,
            RemoveCollateralLongParams {
                collateral_usd: utils::scale(1, USDC_DECIMALS),
                oracle_prices: Some(
                    utils::get_oracle_prices_as_chaos_labs_bundle(&test_setup.program_test_ctx)
                        .await,
                ),
    ...
    // test min leverage
    {
        // Make ETH price to increase in order to make the opened position < min leverage
        test_setup
            .update_oracle_price(
                "eth",
                utils::scale(1_900, Cortex::PRICE_DECIMALS),
                19000000000, // 10bps
            )
            .await;
        ...
        test_instructions::remove_collateral_long(
            ...
            &position_pda,
            RemoveCollateralLongParams {
                collateral_usd: utils::scale(50_000, Cortex::USD_DECIMALS),
                oracle_prices: Some(
                    utils::get_oracle_prices_as_chaos_labs_bundle(&test_setup.program_test_ctx)
                        .await,
                ),

...
pub async fn deep_check_liquidate_position_short() {
...

    {
        ...
        // WHEN ---------------------------------------------------------------
        // Alice: Open 1 ETH short position leverage x40 @ 1500
        ...
        // we don't want any borrow here, we will test that specifically elsewhere
        // utils::warp_forward(&test_setup.program_test_ctx, 1).await;

        // Makes ETH price go up to liquidation level
        test_setup
            .update_oracle_price(
                "eth",
                utils::scale(1535, Cortex::PRICE_DECIMALS),
                15200000000, // 10bps
            )
            .await;

        // Executioner: Liquidate Alice ETH position
        test_instructions::liquidate_short(
            &test_setup.program_test_ctx,
            executioner,
            &test_setup.payer_keypair,
            &test_setup.pool_pda,
        ...
        let collateral_usd_value = 1498500000; //base_eth_price_low - spread for swap
        let real_position_size_usd = collateral_usd_value * leverage;
        let liquidation_fee_usd = 96000000; //real_position_size_usd * 30 / Cortex::BPS_POWER as u64; // 30 bps fee
        ...
        // Check Alice token accounts
        {
            ...
            assert_eq!(alice_usdc_before - alice_usdc_after, 1496103897);

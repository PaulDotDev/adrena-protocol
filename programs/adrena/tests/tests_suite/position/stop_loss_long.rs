use {
    ...
    adrena::{
        instructions::{InitUserProfileParams, OpenPositionLongParams, SetStopLossLongParams},
        state::{
            cortex::Cortex,
...
pub async fn stop_loss_long() {
    ...
    // Martin: Open 10 SOL long position x5
    let position_pda = test_instructions::open_position_long(
        ...
        OpenPositionLongParams {
            ...
            collateral: utils::scale(5, JITOSOL_DECIMALS),
            leverage: 50000, // x5
            oracle_prices: Some(
    ...
    test_instructions::set_stop_loss_long(
        ...
        SetStopLossLongParams {
            ...
            // Lowest price we agree to close the position at
            close_position_price: Some(utils::scale(60, Cortex::PRICE_DECIMALS)),
        },
    )
    .await
    .unwrap();
    ...
    {
        test_setup
            .update_oracle_price("sol", utils::scale(90, Cortex::PRICE_DECIMALS), 0)
            .await;

        utils::warp_forward(&test_setup.program_test_ctx, 10).await;

        // Trigger SL automation
        assert!(
            !utils::execute_stop_loss_long_automation(
                &test_setup.program_test_ctx,
                &position_pda,
                &test_setup.payer_keypair,
                Some(
    ...
    //  Update then Stop loss fails when limit is not reached
    {
        // Martin: set SL on the position
        test_instructions::set_stop_loss_long(
            &test_setup.program_test_ctx,
            ...
            &position_pda,
            SetStopLossLongParams {
                stop_loss_limit_price: utils::scale(79, Cortex::PRICE_DECIMALS),
        ...
        test_setup
            .update_oracle_price("sol", utils::scale(80, Cortex::PRICE_DECIMALS), 0)
            .await;

        utils::warp_forward(&test_setup.program_test_ctx, 10).await;

        // Trigger SL automation - should fail (verify the SL close value to be updated)
        assert!(
            !utils::execute_stop_loss_long_automation(
                &test_setup.program_test_ctx,
                &position_pda,
    ...
    {
        test_setup
            .update_oracle_price("sol", utils::scale(85, Cortex::PRICE_DECIMALS), 0)
            .await;

        utils::warp_forward(&test_setup.program_test_ctx, 1).await;

        // Martin: reset SL on the position with loose slippage
        test_instructions::set_stop_loss_long(
            &test_setup.program_test_ctx,
            martin,
            &test_setup.payer_keypair,
            ...
            SetStopLossLongParams {
                ...
                // Lowest price we agree to close the position at
                close_position_price: Some(utils::scale(80, Cortex::PRICE_DECIMALS)),
            },
        )
        .await
        .unwrap();
